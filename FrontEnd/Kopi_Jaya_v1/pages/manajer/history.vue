        ...
        <v-col cols="12" lg="12" class="mt-3">
          <v-row justify="center">
            <v-btn
              color="red lighten-2"
              class="mb-3 mt-3"
              @click="dialog_tanggal = true"
            >
              Pilih Tanggal
            </v-btn>
            <v-btn class="ml-3 mt-3" @click="reloadPage()"> refresh </v-btn>
            <v-dialog v-model="dialog_tanggal" width="500">
              <v-card>
                ...
                <v-card-actions>
                  ...
                  <v-btn
                    color="primary"
                    text
                    @click="(dialog_tanggal = false), getFromDateTransaksi()"
                  >
                    Sudah
                  </v-btn>
                </v-card-actions>
...
export default {
  ...
  methods: {
    ...
    async getFromDateTransaksi() {
      let tokens = JSON.parse(sessionStorage.getItem("token"));
      this.$axios
        .get("transaksis/filter/date/" + this.picker, {
          headers: {
        ...
        .then((response) => {
          ...
          this.recentTransaksis = [];
          response.data.transaksi.forEach((element) => {
            this.recentTransaksis.push({
              ...
              dtl_transaksi: element.dtl_transaksi
                ...
                .toString(),
              //make to format date
              tgl_transaksi: new Date(element.tgl_transaksi).toLocaleDateString(
                "id-ID"
              ),
